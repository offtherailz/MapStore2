    ...
    action$.ofType(TOGGLE_3D)
        .switchMap( (action) => {
            ...
            return Rx.Observable.of(changeMapType(action.originalMapType !== "cesium" ? "cesium" : last2dMapType))
                // RESTORE MAP TYPE ON LOCATION CHANGE
                .concat(action$.ofType(LOCATION_CHANGE).take(1).switchMap(() => Rx.Observable.of(changeMapType(last2dMapTypeSelector(store.getState())))));
        });
